      ...
      <div className="flex gap-4">
        ...
        <Dialog open={isManageDialogOpen} onOpenChange={setIsManageDialogOpen}>
          ...
          <DialogContent className="sm:max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Current Relationship Types</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              {managingTypes.map((type) => (
                <Card key={type.uuid} className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                    <div>
                      <Label className="text-sm font-medium">Name</Label>
                      <p className="text-sm text-muted-foreground">{type.display}</p>
                    </div>
                    <div>
                      <Label htmlFor={`displayOrder-${type.uuid}`} className="text-sm font-medium">
                        Display Order
                      </Label>
                      <Select
                        value={type.displayOrder.toString()}
                        onValueChange={(value) =>
                          updateManagingType(type.uuid, 'displayOrder', parseInt(value))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id={`preferred-${type.uuid}`}
                        checked={type.preferred}
                        onCheckedChange={(checked) =>
                          updateManagingType(type.uuid, 'preferred', checked)
                        }
                      />
                      <Label htmlFor={`preferred-${type.uuid}`} className="text-sm font-medium">
                        Preferred
                      </Label>
                    </div>
                  </div>
              ...
              <Button onClick={handleSaveManagingTypes} className="w-full">
                <Save className="h-4 w-4 mr-2" />
                Save Changes
              </Button>
              ...
              <div className="space-y-2 pt-4 border-t">
                <p className="text-sm text-muted-foreground">
                  Display order determines the sequence in which relationship types appear in dropdown menus and lists.
                </p>
                <p className="text-sm text-muted-foreground">
                  Preferred relationship types are highlighted and appear at the top of selection lists for better user experience.
                </p>
              </div>
            </div>
          </DialogContent>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {relationshipTypes.map((type) => (
                <div key={type.uuid} className="flex justify-between items-start p-4 border rounded-lg">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-foreground">{type.display}</h3>
                      {type.preferred && (
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                      <span>A is to B: <strong>{type.aIsToB}</strong></span>
                      <span>B is to A: <strong>{type.bIsToA}</strong></span>
                      <span>Display Order: <strong>{type.displayOrder}</strong></span>
                    </div>
                  </div>
                </div>
              ))}
